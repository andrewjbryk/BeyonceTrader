(function($){
	var welcome = {
		...
		'init' : function () {
			...
			$('.ticker__changer').change(function() {
			    var ticker = $(this).val();
			...
			if(pathArray[pathArray.length - 1] == ""){
				$.getJSON("s/homepage.json", function(currentData){
					...
					percentChangeInt = parseFloat(percentChange);
					
					if(percentChangeInt > -2.5 && percentChangeInt <= 2.5){
						//$('#beyonceStatus').text("okay");
						$('#beyonceValue #change').removeClass();
						$('#beyonceImage').attr('src', 'lib/img/okay.gif');
						$('#beyonceImage').show();
					} else if(percentChangeInt > 2.5){
				...
				$.getJSON("api/" + pathArray[pathArray.length - 1] + ".json", function(currentData){
					...
					if(percentChangeInt > -5 && percentChangeInt <= 5){
						//$('#beyonceStatus').text("okay");
						$('#beyonceValue #change').removeClass();
						$('#beyonceImage').attr('src', 'lib/img/okay.gif');
						$('#beyonceImage').show();
						...
						$('#stock_name').show();
					} else if(percentChangeInt > 5){
						//$('#beyonceStatus').text("great");
						$('#beyonceValue #change').removeClass().addClass("great");
						$('#beyonceImage').attr('src', 'lib/img/great.gif');
						$('#beyonceImage').show();
						$("#beyonceName").text(stockName);
